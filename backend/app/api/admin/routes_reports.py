...
async def get_reports_for_management(
    ...
    """제보 관리 목록 조회"""
    data = await admin_report_service.get_reports(skip, limit, status, category, str(assigned_admin_id) if assigned_admin_id else None)
...
async def get_report_detail(
    ...
    """제보 상세 조회"""
    report = await admin_report_service.get_report_detail(str(report_id))
...
async def update_report_status(
    ...
    result = await admin_report_service.update_report_status(
        str(report_id), status_update.status, status_update.admin_comment,
...
async def bulk_report_action(
    ...
    result = await admin_report_service.bulk_report_action(
        [str(rid) for rid in bulk_action.report_ids], bulk_action.action,
