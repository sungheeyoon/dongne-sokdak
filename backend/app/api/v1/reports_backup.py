...
async def get_reports(
    ...
    try:
        ...
        print(f"🔍 검색 결과: {len(reports)}개 제보 (검색어: {search}, 사용자: {user_id})")
        ...
        for report in reports:
            ...
            if report.get("location"):
                try:
                    ...
                    print(f"⚠️ 위치 파싱 실패: {parse_error}")
                    report["location"] = {"lat": 37.5665, "lng": 126.9780}
            else:
                # 기본값 설정
                report["location"] = {"lat": 37.5665, "lng": 126.9780}
        ...
        raise HTTPException(status_code=400, detail=f"조회 오류: {str(e)}")
...
async def update_report(
    ...
    try:
        ...
        # 위치 정보 변환 (PostGIS POINT 형식 파싱)
        # 실제 환경에서는 적절한 파싱 로직이 필요함
        # 예시로 임시 값 할당
        updated_report["location"] = {"lat": 0.0, "lng": 0.0}  # 이 부분은 실제 구현 필요
...
async def get_nearby_reports(
    ...
    try:
        ...
        print(f"📍 근처 제보 조회: {len(nearby_reports)}개 (중심: {lat}, {lng}, 반경: {radius_km}km)")
        return nearby_reports
        
    except Exception as e:
        print(f"❌ 근처 제보 조회 오류: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def get_my_neighborhood_reports(
    ...
    try:
        ...
        print(f"🏠 내 동네 기준 제보 조회: {neighborhood['place_name']} ({lat}, {lng})")
