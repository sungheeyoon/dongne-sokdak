...
from app.schemas.profile import Profile, ProfileUpdate, AvatarUpdate, NeighborhoodUpdate
...
async def get_my_profile(
    ...
    """내 프로필 조회 (통계 포함)"""
    return await profile_service.get_my_profile(current_user_id)
...
async def update_my_profile(
    ...
    """내 프로필 수정"""
    return await profile_service.update_profile(current_user_id, profile_in)
...
async def get_user_profile(
    ...
    result = await profile_service.get_user_profile(str(user_id))
    if result is None:
...
async def update_avatar(
    ...
    """아바타 이미지 업데이트"""
    await profile_service.update_avatar(current_user_id, avatar_data.avatar_url)
...
async def update_my_neighborhood(
    ...
    """내 동네 설정 업데이트"""
    result = await profile_service.update_neighborhood(current_user_id, neighborhood_data)
    return {
...
async def delete_my_neighborhood(
    ...
    """내 동네 설정 삭제"""
    success = await profile_service.delete_neighborhood(current_user_id)
    if not success:
