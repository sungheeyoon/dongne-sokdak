...
async def create_vote(
    ...
    """특정 제보에 공감(투표) 추가"""
    return await vote_service.create_vote(vote_in, current_user_id)
...
async def delete_vote(
    ...
    """특정 제보에 대한 공감(투표) 취소"""
    await vote_service.delete_vote(str(report_id), current_user_id)
    return
...
async def get_vote_count(
    ...
    """특정 제보의 공감 수 조회"""
    count = await vote_service.get_vote_count(str(report_id))
    return {"count": count}
...
async def check_vote(
    ...
    voted = await vote_service.check_vote(str(report_id), current_user_id)
    return {"voted": voted}
