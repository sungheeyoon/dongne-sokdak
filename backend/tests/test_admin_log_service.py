import pytest
from app.services.admin.log_service import AdminLogService
...
async def test_get_admin_activity_logs_success(mocker):
    ...
    service = AdminLogService(mock_supabase)
    result = await service.get_admin_activity_logs()
...
async def test_get_admin_activity_logs_filter_action(mocker):
    ...
    service = AdminLogService(mock_supabase)
    await service.get_admin_activity_logs(action="update_role")
