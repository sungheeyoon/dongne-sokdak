...
- **Component Naming**: Enforce `Ui` prefix for generic UI atoms (e.g., `UiButton`, `UiInput`) to avoid HTML/Library conflicts.
...
**Objective**: Eliminate naming ambiguity between HTML elements and Custom UI components.

1. **Prefixing (Mandatory)**:
   - All generic UI atoms MUST use the `Ui` prefix.
   - Examples: `UiButton`, `UiInput`, `UiForm`, `UiCard`, `UiBadge`.
   - **Reasoning**: Prevents collision with HTML tags (`<button>`, `<input>`, `<form>`) and other libraries.

2. **Disambiguation**:
   - Do NOT use raw names like `Button` or `Input` even if they are just wrappers.
   - If wrapping a library component (e.g., `react-hook-form`), use a distinct name (e.g., `UiForm` or `AppForm`).

3. **Feature Components**:
   - Use descriptive, domain-specific names.
   - Examples: `LoginButton`, `SearchInput`, `ReportCard`.
